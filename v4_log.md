...
**Spread definition — this is the part that had to be fixed before coding.**
An earlier draft doubled up toxicity compensation by setting the adaptive
base spread to BestFixed (h=1.08, which already contains the time-averaged
toxicity premium) and then adding phi_hat on top. Corrected:
...
base is 1/kappa = 1.00, not 1.08 — BestFixed does not reappear in the
formula. Locked parameters: k=0.04, N=50, lambda=1.
...
catastrophe under default parameters; one `competition_baseline` parameter
set frozen. This bot is kept permanently — even if the rest of v4 stalls,
the competition has something to run.
...
Competition is 2026-08-17. v4pre is required before then; v4a is attempted
if time allows. v4b onward proceeds afterward at whatever pace, ending in a
GitHub report covering v2 through v4d.
